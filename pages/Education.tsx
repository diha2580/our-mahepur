...
import { GraduationCap, MapPin, Phone, Search, Building, BookOpen, School } from 'lucide-react';
    ...
    <div className="animate-in slide-in-from-right duration-500 pb-16">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
        ...
        <div className="relative group w-full md:w-80">
          ...
          <input 
            ...
            className="pl-12 pr-6 py-3 bg-white border border-gray-100 rounded-2xl shadow-sm outline-none focus:ring-2 focus:ring-indigo-100 transition-all w-full text-black"
          />
        </div>
      </div>
        ...
        {filtered.map((item) => (
          <div key={item.id} className="bg-white rounded-[2rem] overflow-hidden shadow-sm border border-gray-100 hover:shadow-xl transition-all p-8 flex flex-col group">
            ...
            <div className="space-y-4 mb-6">
              ...
              <a 
                href={`tel:${formatPhoneForDialer(item.phone)}`}
            ...
            <button className="mt-auto w-full py-3 bg-indigo-50 text-indigo-600 rounded-xl font-bold text-sm hover:bg-indigo-600 hover:text-white transition-all">
              বিস্তারিত প্রোফাইল
            </button>
          </div>
        ))}
      ...
      {filtered.length === 0 && (
        <div className="py-20 text-center bg-white rounded-3xl border border-dashed border-gray-200">
          <p className="text-gray-400 font-bold">কোনো প্রতিষ্ঠান খুঁজে পাওয়া যায়নি।</p>
        </div>
      )}
    </div>
  );
