...
import { FileText, ArrowRight, MousePointer2, Info, ArrowLeft } from 'lucide-react';
import { eApplications } from '../data';

interface EApplicationsProps {
  onBack: () => void;
}

const EApplications: React.FC<EApplicationsProps> = ({ onBack }) => {
  return (
    <div className="max-w-4xl mx-auto animate-in fade-in duration-500">
      <button 
        onClick={onBack}
        className="mb-6 flex items-center gap-2 text-gray-500 hover:text-teal-600 font-bold transition-colors group"
      >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {eApplications.map((app) => (
          <a key={app.id} href={app.link} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-6 bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-teal-200 group transition-all">
            <div className="flex items-center space-x-4">
              <div className="bg-teal-50 text-teal-600 p-3 rounded-xl group-hover:bg-teal-600 group-hover:text-white transition-colors"><MousePointer2 className="w-6 h-6" /></div>
      ...
      <div className="mt-12 bg-white p-6 rounded-2xl border border-gray-100 flex items-center justify-between">
         <div className="flex items-center space-x-4">
            <div className="bg-yellow-100 text-yellow-700 p-3 rounded-full"><Info className="w-6 h-6" /></div>
