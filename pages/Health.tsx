    ...
    <div className="animate-in slide-in-from-right duration-500 pb-16">
      ...
      {filteredHospitals.length > 0 ? (
            ...
            <div key={hospital.id} className="bg-white rounded-[2rem] overflow-hidden shadow-sm border border-gray-100 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 flex flex-col">
              ...
              <div className="p-8 flex-grow text-black">
                <div className="flex items-start justify-between mb-6">
                   <div className="bg-gray-50 p-4 rounded-2xl"><HospIcon className="w-8 h-8 text-gray-700" /></div>
                ...
                <div className="space-y-4 mb-8">
                  <div className="flex items-start group">
                    <div className="bg-gray-50 p-2 rounded-lg mr-3 group-hover:bg-green-50 transition-colors"><MapPin className="w-4 h-4 text-gray-400 group-hover:text-green-600" /></div>
                    <span className="text-sm font-medium text-gray-600">{hospital.location}</span>
                  </div>
                  <a href={`tel:${formatPhoneForDialer(hospital.phone)}`} className="flex items-center group p-3 bg-green-50 rounded-2xl hover:bg-green-600 transition-all active:scale-95">
                    <div className="bg-white p-2 rounded-xl mr-3 shadow-sm group-hover:text-green-600"><Phone className="w-5 h-5 text-green-600" /></div>
                ...
                <div className="flex flex-wrap gap-2">
                  {(hospital.speciality || []).map((s: string) => (
                    <span key={s} className="bg-gray-50 text-gray-600 text-xs px-3 py-1.5 rounded-xl font-bold border border-gray-100 group-hover:border-green-100 transition-colors">{s}</span>
                  ))}
                </div>
              </div>
              <div className="p-8 pt-0 mt-auto">
                <button className="w-full py-4 bg-gray-900 text-white hover:bg-black rounded-2xl text-sm font-bold transition-all flex items-center justify-center gap-2 shadow-xl shadow-gray-200 active:scale-95">
                  <UserRound className="w-4 h-4" /><span>ডাক্তারদের অ্যাপয়েন্টমেন্ট</span>
                </button>
              </div>
            </div>
          ))}
        ...
        <div className="py-20 text-center bg-white rounded-3xl border-2 border-dashed border-gray-200 animate-in fade-in duration-500">
          <div className="bg-gray-50 w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6"><Search className="w-12 h-12 text-gray-300" /></div>
          <h3 className="text-2xl font-bold text-gray-600 mb-2">দুঃখিত, কোনো হাসপাতাল খুঁজে পাওয়া যায়নি।</h3>
          <p className="text-gray-400 mb-6 max-w-sm mx-auto">"{searchQuery}" এর জন্য কোনো ফলাফল নেই।</p>
          <button onClick={() => {setFilter('All'); clearSearch();}} className="bg-green-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-green-700 transition-all shadow-lg shadow-green-100 active:scale-95">অনুসন্ধান মুছুন এবং পুনরায় দেখুন</button>
        </div>
      )}
    </div>
  );
